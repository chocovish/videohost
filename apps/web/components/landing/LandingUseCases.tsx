  ...
  return (
    <section className="w-full py-16 sm:py-24 relative overflow-hidden" id="use-cases">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-12">
        ...
        <div className="w-full max-w-4xl mx-auto p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-[hsl(var(--border))] overflow-x-auto">
              ...
              return (
                <button
                  ...
                  className={`py-3 px-3.5 rounded-xl text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap text-center ${
                    isSelected
                      ? "bg-white dark:bg-slate-950 text-[hsl(var(--foreground))] shadow-md border border-[hsl(var(--primary))]/30 ring-1 ring-[hsl(var(--primary))]/20"
                      : "text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] hover:bg-black/5 dark:hover:bg-white/5"
                  }`}
                >
                  <Icon className={`w-4 h-4 shrink-0 ${isSelected ? "text-[hsl(var(--primary))]" : ""}`} />
        ...
        <div className="rounded-3xl border border-[hsl(var(--border))] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 sm:p-10 shadow-2xl max-w-4xl mx-auto animate-in fade-in zoom-in-95 duration-200">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
            <div className="md:col-span-8 space-y-5">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-[hsl(var(--primary))]/15 text-[hsl(var(--primary))] text-xs font-black">
                {activePersona.title}
              </div>

              <h3 className="text-xl sm:text-2xl font-black tracking-tight text-[hsl(var(--foreground))] leading-snug">
                {activePersona.headline}
              </h3>

              <ul className="space-y-3 pt-2">
                {activePersona.benefits.map((b, i) => (
                  <li key={i} className="flex items-start gap-3 text-xs sm:text-sm text-[hsl(var(--muted-foreground))] font-medium">
            ...
            <div className="md:col-span-4 flex flex-col justify-center items-center md:items-start p-6 rounded-2xl bg-[hsl(var(--primary))]/10 border border-[hsl(var(--primary))]/20 space-y-4 text-center md:text-left">
              <span className="text-xs font-extrabold uppercase tracking-wider text-[hsl(var(--primary))]">
                Ready to level up?
              </span>
              <p className="text-xs text-[hsl(var(--foreground))] font-bold">
                Get started today with 2GB free cloud storage included.
              </p>
              <Link
                href={activePersona.ctaLink}
                className="w-full py-3 px-4 bg-[hsl(var(--primary))] text-white font-extrabold text-xs rounded-xl shadow-lg hover:opacity-90 transition-all flex items-center justify-center gap-2"
              >
